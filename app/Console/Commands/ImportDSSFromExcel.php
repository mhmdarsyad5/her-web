...
class ImportDSSFromExcel extends Command
{
    ...
    protected function mapItemToConditions(array $item): array
    {
        ...
        if (! empty($item['Operator Type'])) {
            ...
            if (str_contains($opType, 'seated')) {
                $conditions['operator'] = 'seated';
            } elseif (str_contains($opType, 'stand') || str_contains($opType, 'pedestrian')) {
                $conditions['operator'] = 'standing';
            }
        }

        // Energy (from Category or Type)
        $fullText = strtolower(($item['Category'] ?? '').' '.($item['Type'] ?? ''));
        if (str_contains($fullText, 'lithium')) {
            $conditions['energy'] = 'lithium';
        } elseif (str_contains($fullText, 'electric') || str_contains($fullText, 'lead acid')) {
            $conditions['energy'] = 'electric';
        } elseif (str_contains($fullText, 'diesel') || str_contains($fullText, 'ic')) {
            $conditions['energy'] = 'diesel';
        }
        ...
        if (isset($conditions['energy'])) {
            if ($conditions['energy'] === 'diesel') {
                ...
                if (str_contains($fullText, 'rough terrain')) {
                    $conditions['location'][] = 'rough';
                }
                if ($conditions['energy'] === 'lithium') {
